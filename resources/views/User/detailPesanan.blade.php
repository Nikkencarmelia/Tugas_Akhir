...
<head>
  ...
  <style>
    ...
    .detail-header h3::before {
      content: "\f291"; font-family: "Font Awesome 6 Free";
...
<div class="container py-5">
  ...
  <div class="detail-header">
    <div class="d-flex align-items-center gap-3">
      <a href="/riwayat" class="btn btn-outline-secondary btn-sm p-1"><i class="fas fa-chevron-left"></i></a>
      <h3>Detail Pesanan</h3>
    </div>
    <small class="text-muted">Informasi pesanan & pengiriman</small>
  </div>
  ...
  <div class="detail-card">
    ...
    <div class="product-item">
      ...
      <div class="product-details">
        ...
        <p class="small text-muted mb-1">Jumlah: {{ $item['jumlah'] }}</p>
        <p class="small harga-item mb-0">Harga: Rp {{ number_format($item['harga'], 0, ',', '.') }} / item</p>
        <p class="small text-success fw-semibold mb-0">Subtotal: Rp {{ number_format($item['harga'] * $item['jumlah'], 0, ',', '.') }}</p>
      </div>
    </div>
    @endforeach

    @php
      $ongkir_numeric = (int) str_replace(['Rp ', '.', ' '], '', $detail['ongkir']);
      $total = $subtotal + $ongkir_numeric;
    @endphp

    <div class="total-section">
      <p class="mb-0 fw-semibold">Subtotal Produk</p>
      <p class="mb-0 fw-bold">Rp {{ number_format($subtotal, 0, ',', '.') }}</p>
    </div>
    ...
    <div class="total-section">
      ...
      <p class="mb-0 fw-bold text-primary fs-5">Rp {{ number_format($total, 0, ',', '.') }}</p>
    </div>
