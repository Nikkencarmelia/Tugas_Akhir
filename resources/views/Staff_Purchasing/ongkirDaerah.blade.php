...
<style>
    ...
    h1, h2, h3, h4, h5, h6, .nav-link, .btn, .orders-tab {
    ...
    @media(max-width:768px){
        ...
        .select-controls{flex-wrap:wrap;gap:0.5rem;justify-content:flex-start;}
        .select-controls .input-group{max-width:200px !important;}
        .select-controls .form-select{max-width:140px !important;}
    }
        ...
        <div class="tab-panel {{ $activeTab == 'kecamatan' ? 'active' : '' }}" id="kecamatan">
            ...
            <div class="search-controls mb-3">
                <div class="input-group flex-grow-1 search-wrapper">
                    ...
                    <input type="text" id="kecamatanSearch" class="form-control" placeholder="Cari kecamatan...">
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered align-middle" id="tableKecamatan">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-3">Tidak ada data kecamatan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="table-responsive">
                <table class="table table-bordered align-middle" id="tableKelurahan">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-3">Tidak ada data kelurahan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="table-responsive">
                <table class="table table-bordered align-middle" id="tableKodePos">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-3">Tidak ada data kode pos.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Robust Live Search with Instant Filtering
        function initLiveSearch(inputId, tableId, filterConfig = []) {
            ...
            const performFilter = () => {
                ...
                const filterValues = filterElements.map(f => ({
                    val: f.element ? f.element.value : '',
                    attr: f.attr
                }));
                ...
                rows.forEach(row => {
                    // Search in non-action cells
                    const cellsToSearch = Array.from(row.cells).slice(0, -1);
                    const rowText = cellsToSearch.map(c => c.innerText).join(' ').toLowerCase();
                    
                    const matchesSearch = rowText.includes(term);
                    const matchesFilters = filterValues.every(f => {
                        if (f.val === '') return true;
                        return row.getAttribute(f.attr) === f.val;
                    });
                ...
                if (!hasVisible) {
                    if (!noResultsRow) {
                        ...
                        noResultsRow.innerHTML = `<td colspan="${table.rows[0].cells.length}" class="text-center py-4 text-muted">
