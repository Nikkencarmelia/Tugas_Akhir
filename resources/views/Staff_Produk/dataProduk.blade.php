                        ...
                        <div class="d-flex gap-2 flex-wrap align-items-center filters-vertical">
                            ...
                            <select id="filter-supplier" name="supplier" class="form-select" style="width:150px"
                                ...
                                @foreach ($supplier as $sp)
                                    <option value="{{ $sp->id }}"
                                        {{ request('supplier') == $sp->id ? 'selected' : '' }}>
                                        {{ $sp->nama_supplier }}
                                    </option>
                                @endforeach
                            </select>
                                ...
                                @else
                                    @foreach($produk as $p)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $p->jumlah_satuan }} {{ $p->satuan->nama_satuan ?? '-' }}</td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                <button class="btn btn-danger btn-delete" onclick="handleDelete({{ $p->id }},'{{ $p->nama_produk }}',{{ $batchCount }})">
                                                    <i class="bi bi-trash"></i>
                                                </button>

                                                <a href="{{ route('produk.batch.index', $p->id) }}" class="btn-action btn-batch">
        ...
        <div class="modal-dialog modal-dialog-centered">
            ...
            <script>
                ...
                function handleDelete(id,nama,batchCount){
                    if(batchCount>0){
                        new bootstrap.Modal(
                            document.getElementById('batchWarningModal')
                        ).show();
                        return;
                    }
