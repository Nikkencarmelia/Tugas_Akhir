...
<head>
    ...
    <style>
        ...
        .img-preview{width:120px;height:120px;border-radius:10px;object-fit:cover;border:1px solid #ddd;margin-top:10px;display:none}
        .current-image{max-width:200px;height:auto;border-radius:10px;border:1px solid #ddd;margin-top:10px;display:block}
    </style>
</head>
<body>
...
<div class="container-produk">
    ...
    <form action="{{ route('produk.update', $produk->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" name="estimasi_kadaluwarsa_hari" min="1" value="{{ old('estimasi_kadaluwarsa_hari', $produk->estimasi_kadaluwarsa_hari) }}" oninput="if(this.value === '0') this.value = '';">
...
const nameInput = document.getElementById('nama_produk');
        ...
        if (nameInput && nameWarning) {
            nameInput.addEventListener('input', function() {
                ...
                timeout = setTimeout(() => {
                    fetch(`{{ route('produk.check-name') }}?name=${encodeURIComponent(name)}&exclude_id={{ $produk->id }}`)
